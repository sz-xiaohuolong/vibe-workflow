...
只有真正的 Release 才拥有上述目录；同一 Release 的 S1/S2/S3 记录在 `IMPLEMENTATION_PLAN.md` 的 Slice Map 和 living `PROGRESS.md` 中。每个 Slice 在 PROGRESS 清单中用一条记录写状态、证据链接和下一步，不为 Slice 新建 `releases/<release-id>-Sx/`、单独子目录或整套 SPEC/PLAN/REVIEW/VERIFICATION。复杂切片确需调查、设计或决策文件时，按实际职责添加单份文件并链接，不能因此升级为 Release。已有历史目录不自动搬迁或改写；在 Document Map 中标明其历史性质，后续按真实 Release 边界写入。
...
`PROGRESS.md` 与 `SPEC.md` 的标题后先放最多三条数据行的摘要表，列出范围/当前事实、阻塞或边界、下一步或验收入口；首屏不写长段状态辩护。摘要是指向下方事实的导航，不复制整段明细；相同状态只在一张明细表或一个字段维护。PROGRESS 摘要报告当前执行与验证事实；SPEC 摘要报告冻结的产品范围和验收入口，不复制随测试变化的运行进度。正文按读者当前要完成的事组织：进度供快速定位，SPEC 供核对行为，操作步骤和背景解释各归其职责；不要机械建立四类空目录。复制模板后删除不适用的空章节和占位符，保留适用 Gate 所需事实，不为“完整”生成空记录。
...
新生成的测试日志、运行 dump、原始指标 JSON 等暂存于项目根目录 `.evidence/<release-id>/<slice-or-task-id>/`，默认在目标项目 `.gitignore` 加入 `/.evidence/`；未建立 Release 的 `init` 审计或 Tiny Task 可用 `.evidence/init/` 或 `.evidence/task-<id>/`，并记录实际路径。沿用已有等价 `artifacts/`/CI artifact 位置时，在 Document Map 或 VERIFICATION 标明即可。`docs/` 保存人类可读的 Markdown 摘要、结果、命令、时间、环境、退出码及证据指针，不内嵌整段日志。配置、Schema、源数据和需版本控制的 JSON 并非“原始运行证据”，不能仅凭扩展名迁走或忽略。

忽略本地证据不等于丢失可追溯性。凡支撑 Release `VERIFIED`、`READY_TO_SHIP` 或长期审计的结果，VERIFICATION 必须记录可长期取回的证明（如 CI 运行链接与运行 ID、明确保留期的 artifact，或有意纳入版本控制的证据快照）及对应 commit；无 Git 时记录稳定 artifact ID、内容摘要与可恢复版本，不伪造 commit。不可重建的原始产物须在声明前稳定归档。只有 `.evidence/` 本机路径、没有远程可取回证明时，不得宣称该证据已可供跨机器复核。接管旧仓库时先识别既有引用和保留要求，不自动移动、删除旧 `docs/` 证据。
...
| 变化 | 必须更新 | 按需更新 |
|---|---|---|
| 新 Release（包括已封存版本的后续修复） | PROJECT_BRIEF、SPEC、PLAN、PROGRESS | PROPOSED_DESIGN、DEC、CHANGE；实现后按证据更新 TECH_DESIGN |
| 当前 Release 内新 Feature/Slice | PROGRESS 清单一条；PLAN 的 Slice Map 按需更新 | 影响既有 REQ/AC 时更新对应验证行；无独立文档套件；若改变冻结范围，先过 Requirement Change Gate |
