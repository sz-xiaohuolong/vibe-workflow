...
对已发布产品的 Bug 修复也创建新的 patch Release 或后续 Release；旧 Release 只保存当时事实。这个封存边界优先于 Tiny 的轻文档预算：Tiny patch Release 仍保留新 Release 必需的四份事实职责，但每份只写这次修复所需的最小内容，不生成空的可选文件，也不能回写已发布版本。
